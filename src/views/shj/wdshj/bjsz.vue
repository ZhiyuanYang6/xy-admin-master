    ...
    <el-tabs type="card" @tab-click="handleClick">
      <el-tab-pane v-for="item in tabslx" :key="item.label" :label="item.label">
        <el-table :data="tableData" border style="width: 100%">
          ...
          <el-table-column prop="yjyz" label="报警阀值" width="150">
            <template slot-scope="scope">
              <el-input v-model="scope.row.num" suffix-icon="iconfont icon-shuzishurukuang" style="width: 120px;" size="mini" placeholder="请输入数量"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="是否支持订货" width="150">
            <template slot-scope="scope">
              <el-select v-model="scope.row.num" placeholder="请选择" size="mini">
    ...
    submitsetdetil: function() {
      ...
      for (var i= 0; i<this.tableData.length; i++) {
       arr.push(this.tableData[i]);
        ...
        console.log(parm);
        //debugger;
         request({ url: '/shjgl/editHdbj', method: 'post', data: parm })
        .then(response => {
          this.loading = false;
