<template>
  <div class="smain">
    ...
    <el-dialog title="选择商品" :visible.sync="dialogspxx" width="60%" append-to-body class="wdshjspsz">
      ...
      <el-table :data="spData"   max-height="250"  border highlight-current-row @current-change="handleCurrentspChange" style="width: 100%; cursor: pointer;" border>
    ...
    <el-dialog title="选择模板" :visible.sync="dialogxzmb" width="60%" append-to-body class="wdshjspsz">
      ...
      <div class="mbbtn">
        <el-button type="primary" @click="impotrMbxx()">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    mbxx: function(val) {
      ...
      request({ url: 'service-machine/mbgl/mbglQuery', method: 'post', data: querymbxx }).then(response => {
        ...
        var querymb = { mbid:this.mbid,jqbh:this.listrow.jqbh}
      request({ url: 'service-machine/shjgl/queryspmbxx', method: 'post', data: querymb }).then(response => {
      this.tabeldates = response;
        for (var i = 0; i < this.listrow.jgsl; i++) {
          if (this.oldtab == "副机" + i) {
            this.tableData = this.tabeldates[i];
